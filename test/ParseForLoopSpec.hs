...
import Parser (parseForLoop, parseForLoopHeader, parseUpdateStatement, extractForLoopHeader, ensureSemicolon)
...
spec = do
  describe "extractForLoopHeader" $ do
    it "successfully extracts content inside parentheses" $ do
      let tokens = [TFor, TRparen, TInt, TIdent "i", TAssign, TIntLit 0, TSemicolon, TIdent "i", TLessThan, TIntLit 10, TSemicolon, TIdent "i", TIncrement, TRparen, TLbrace]
      extractForLoopHeader (drop 2 tokens) `shouldBe` Right ([TInt, TIdent "i", TAssign, TIntLit 0, TSemicolon, TIdent "i", TLessThan, TIntLit 10, TSemicolon, TIdent "i", TIncrement], [TLbrace])
  ...
  describe "parseForLoopHeader" $ do
    it "parses valid for loop headers correctly" $ do
      let tokens = [TInt, TIdent "i", TAssign, TIntLit 0, TSemicolon, TIdent "i", TLessThan, TIntLit 10, TSemicolon, TIdent "i", TIncrement, TRparen, TLbrace]
